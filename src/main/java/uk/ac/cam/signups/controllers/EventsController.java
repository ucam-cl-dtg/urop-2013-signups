...
public class EventsController extends ApplicationController {
	...
	@Path("/queryPupils")
	@Produces(MediaType.APPLICATION_JSON)
	public Map<String, ?> queryPupils(@QueryParam("page") int page,
			@QueryParam("partial") String partial) {
	...
	public Map<String, String> deleteEvent(
		...
		try {
			...
			logger.warn(e.getCrsid()
					+ " tried to delete an event without owning it.");
			return ImmutableMap.of("error",
					"You cannot delete an event that you do not own.");
		}
	...
	public Map<String, ?> generateIndividualsRows(@QueryParam("page") int page,
		...
		try {
			...
			if (isHisPupil) {
				ImmutableMappableExhaustedPair<Row> rows = u.getRowsSignedUp(
						page, "dos");
				return ImmutableMap
	...
	public Object getCalendar() throws SocketException, URISyntaxException {
		...
		for (Event event : events) {
			for (Row row : event.getRows()) {
				...
				ev.getProperties().add(
						new Description("Event group: "
								+ Util.join(supervisees, ", ")));
		...
		for (Row row : rows) {
			...
			Dur dur = new Dur(start, end);
			String subject = "OTTER: "
					+ row.getEvent().getTitle()
					+ (row.getType() != null ? " (" + row.getType().getName()
							+ ")" : "");
			String hostMail = row.getEvent().getOwner().getCrsid()
					+ "@cam.ac.uk";
			UidGenerator ug = new UidGenerator("1");
		...
		builder.header("Content-Disposition",
				"attachment; filename=events_for_" + cu.getCrsid() + ".ics");
